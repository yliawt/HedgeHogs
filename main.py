...
@app.route('/chatbox')
def chatbox():
    return render_template(('3ChatBox.html'))
...
def perpuluhan_latihan_game():
    return render_template('Perpuluhan_latihan_game.html')


@app.route('/perpuluhan/kuiz')
def perpuluhan_kuiz():
    return render_template('Perpuluhan_kuiz.html')
...
def upload_exercise():
    cur = mysql.connection.cursor()
    cur.execute("SELECT * FROM materials WHERE type = 'exercise' ")
...
def upload_quiz():
    return render_template(('2lecPilihTopik_TambahKuiz.html'))
...
def signup():
    ...
    if request.method == "POST":
        ...
        role = "Pelajar"
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO profile (id, password, role) VALUES (%s, %s, %s)", (id, password, role))
...
app.route('/upload/quiz', method=["POST"])
def upload_quiz():
    if request.method == "POST":
        topic = request.form['topic']
