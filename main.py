...
app.config['MYSQL_DB'] = 'silent math genius'
mysql = MySQL(app)
...
def selectedChatBox():
    ...
    cur = mysql.connection.cursor()
    cur.execute("SELECT roomcode FROM users WHERE id = %s", (username,))
...
def stdmaterial():
    id=session["username"]
    cur=mysql.connection.cursor()
    cur.execute("SELECT perpuluhan, tingkatan3, tingkatan4 FROM score WHERE id=%s",(id,))
    score=cur.fetchall()
...
def perpuluhan_latihan_game():
    cur = mysql.connection.cursor()
    cur.execute("SELECT words, soalan, hint FROM permainan WHERE topik='Perpuluhan'")
    game = cur.fetchall()
...
def tingkatan3_latihan_game():
    cur = mysql.connection.cursor()
    cur.execute(
        "SELECT words, soalan, hint FROM permainan WHERE topik='Tingkatan 3: Matematik Pengguna I'")
    game = cur.fetchall()
...
def tingkatan4_latihan_game():
    cur = mysql.connection.cursor()
    cur.execute(
        "SELECT words, soalan, hint FROM permainan WHERE topik='Tingkatan 4: Matematik Pengguna II'")
    game = cur.fetchall()
...
def upload_exercise():
    ...
    cur.execute("SELECT * FROM permainan")
...
def delete_permainan():
    ...
    cur = mysql.connection.cursor()
    cur.execute("DELETE FROM permainan WHERE soalan=%s", (soalan,))
    mysql.connection.commit()
...
def signup():
    if request.method == "POST":
        ...
        role = "Pelajar"
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO users (id, password, role, roomcode) VALUES (%s, %s, %s, %s)",
                    (id, password, role, room_code))
        cur.execute("INSERT INTO profile (id) VALUES (%s)",(id,))
        cur.execute("INSERT INTO score (id) VALUES (%s)", (id,))
        mysql.connection.commit()
        session['username'] = id
...
def login():
    if request.method == "POST":
        ...
        cur.close()
        if role== None:
            flash("Informasi tidak sah. Sila periksa sekali ID, kata laluan, dan peranan anda.")
            session['flash_message'] = True  # Store a flag in session to indicate a flash message
            ...
            if password == check_password[0] and role[0] == option:
                ...
                cur = mysql.connection.cursor()
                cur.execute("SELECT roomcode,role FROM users WHERE id = %s", (id,))
                roomcode_result = cur.fetchone()
                roomcode = roomcode_result[0]
...
def insert():
    ...
    if request.method == "POST":
        ...
        if "name" in request.form or "photo" in request.form:
            ...
            cur.execute(
                "UPDATE profile SET birth=%s, gender=%s, phone=%s, email=%s, school=%s, address1=%s, address2=%s, poscode=%s, negeri=%s WHERE id = %s",
...
def upload_materials():
    if request.method == "POST":
        ...
        if category == "Permainan":
            ...
            cur = mysql.connection.cursor()
            cur.execute(
                "INSERT INTO permainan (topik, words, soalan, hint) VALUES (%s, %s, %s, %s)",
                (topic, words, soalan, hint))
            mysql.connection.commit()
        else:
            filename = request.form['filename']
            description = request.form['description']
...
def store_quiz_results():
    ...
    if category == "perpuluhan":
        cur.execute("SELECT perpuluhan FROM score WHERE id=%s", (id,))
        ori = cur.fetchone()
        if score > ori[0]:
            cur.execute("UPDATE score SET perpuluhan=%s WHERE id=%s", (score, id))
    elif category == "tingkatan3":
        print("testing")
        cur.execute("SELECT tingkatan3 FROM score WHERE id=%s", (id,))
        ori = cur.fetchone()
        if score > ori[0]:
            cur.execute("UPDATE score SET tingkatan3=%s WHERE id=%s", (score, id))
    elif category == "tingkatan4":
        cur.execute("SELECT tingkatan4 FROM score WHERE id=%s", (id,))
        ori = cur.fetchone()
        if score > ori[0]:
            cur.execute("UPDATE score SET tingkatan4=%s WHERE id=%s", (score, id))
    mysql.connection.commit()
    response_data = {
